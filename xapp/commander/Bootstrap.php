...
class XApp_Commander_Bootstrap extends XApp_Bootstrap implements Xapp_Singleton_Interface
{
	...
	public function handleRequest()
	{
		...
		if (self::isRPC()) {
			...
			//error_log('req client');
			//Restore user from session before client rendering.
			$um = $this->getUserManager();
			if ($um) {
				$user = $um->getUser();
				if ($user !== null) {
					$res = $um->login($user->getName(), $user->getPassword());
	...
	public function render($print = true)
	{
		...
		if ($this->isLoggedIn()) {
			...
			$RPC_TARGET = xo_get(XApp_App_Renderer::SERVICE_URL,$this->appRenderer);
			$this->resourceRenderer->registerRelative('RPC_TARGET', $RPC_TARGET);
...
// Track page view
	...
	public function testStore($storeService)
	{


		/*
		$obj = '{"id":"anotherTest","value":4}';
		$data = $storeService->set('settings','.',null,$obj);
		*/
		/*$storeService->get('settings',null);*/
		/*$data = $storeService->get('settings','.',null);
		error_log('data : ' . json_encode($data));*/

		/*xapp_dumpObject($data);*/

		/*
		$obj = '{"name":"test","value":3}';
		$data = $storeService->set('settings','.',null,$obj);

		$obj = '{"name":"anotherTest","value":4}';
		$data = $storeService->set('settings','.',null,$obj);*/


		/*
		$obj = '{"id":"idDriven","value":99}';
		$data = $storeService->set('settings','/settings',null,$obj);
		*/


		/*
		$obj = '{"value":123123}';
		$data = $storeService->set('settings','.','{"name":"test2"}',$obj);
		*/

		//implicit
		/*$obj0 = '{"value":45555}';
		$data = $storeService->set('settings','.','{"name":"test4"}',$obj0);*/
	...
	public function setup()
	{
		...
		if ($javascriptPlugins && count($javascriptPlugins)) {
			...
			$resourceVariables = (array)$xappResourceRenderer->registryToKeyValues(
